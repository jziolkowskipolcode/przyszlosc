...
if (!isset($_POST['submit'])) {
  ...
  if(!empty($_POST['email'])) {
  	...
  	if (!validEmail($email[2])) {
  	  ...
  	  $email[4] = '<strong><span style="color:#FF0000;">Nieprawidłowy email</span></strong>';
	  }
  }
  else {
    $error = 1;
  ...
  if(empty($_POST['captcha_code'])) {
    ...
    if(!$valid) {
      $error = 1;
      $code[3] = 'color:#FF0000;';   
      $code[4] = '<strong><span style="color:#FF0000;">Nieprawidłowy kod</span></strong>';
    }
  }

  if ($error == 1) {
    $error_message = '<span style="font-weight:bold;font-size:90%;">Proszę poprawić/wypełnić pola zaznaczone na czerwono.</span>';
    ...
    if(!mail($to,"$subject_prefix - $subject[2]", $body, $headers)){
    echo "Błąd!!";
    }else{
...
<table style="width:100%; height:{$form_height}; background:{$form_background}; border:{$form_border_width} {$form_border_style} {$form_border_color}; padding:10px; margin-left:30px;" id="contactForm">
<tr>
...
<br /><br />(Wpisz tekst widoczny na obrazku. Wielkość liter nie ma znaczenia.)<br />
<a href="#" onclick="document.getElementById('captcha').src = 'formfiles/contact-securimage_show.php?' + Math.random(); return false">Kliknij tutaj jeśli masz problem z odczytaniem znaków.</a>
</td>
</tr>
<tr>
<td colspan="2" style="text-align:left; vertical-align:middle; padding:{$cell_padding}; font-size:90%; font-weight:bold;">Wymagane jest wypełnienie wszystkich pól.</td>
</tr>
<tr>
<td colspan="2" style="text-align:left; vertical-align:middle; padding:{$cell_padding};"><input type="submit" name="submit" value="Wyślij" style="border:1px solid #999;background:#E4E4E4;margin-top:5px;" id="submit_button" /></td>
</tr>
</table>
</form>
